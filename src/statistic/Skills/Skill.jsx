...
function Skill({ name, id, ability, hasProf, hasExp, updateSkill }) {
  ...
  function handle(bonus, name) {
    let result = Number.parseInt(Math.random() * 20 + 1);
    alert(
      `
      -=-=-=-=-= ${name} Check =-=-=-=-=-
  ...
  return (
    <div className="skill">
      ...
      <div className="skillInfo">
        ...
        <p
          onClick={() =>
            handle(modCalc(abilityContext[ability - 1].score + bonus * 2), name)
          }
          style={{ cursor: "pointer" }}
        >
          {sign(modCalc(abilityContext[ability - 1].score) + bonus)}
        </p>
      </div>
    </div>
  );
